...
最终现场复核（20:40:40，Asia/Shanghai）：源内容读取成功 58、转换成功 6、成功步骤总数 373；已发布文档增至 5，其中近 10 分钟新增 2 份，最近一次发布为 20:37:10。同步队列仍为 2 个执行中、287 个等待。存在端到端进展，但队列尚未清空。两台模型自监控修复后近 12 分钟新增鉴权错误仍均为 0，指标采集 `up` 均为 1。本次补记只更新验收记录，不修改运行配置。
