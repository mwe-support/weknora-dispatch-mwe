...
const metrics = (row: HistoryRow) => row.kind === 'legacy'
  ? [['runId',row.run_id],['originalErrorOrdinal',row.original_error_ordinal],['originalErrorDigest',row.original_error_digest],['legacyAction',row.legacy_action],['legacyActor',row.legacy_actor],['legacyDeadLetter',row.legacy_dead_letter_id],['originalResult',row.legacy_run_status]].filter(([,value]) => value != null && value !== '')
  : view.value === 'current_run_progress'
    ...
    <template v-else>
      ...
      <template v-if="page">
        ...
        <div v-else class="table-wrap"><table><thead><tr><th>{{ t('processing.document') }}</th><th>{{ t('processing.version') }}</th><th>{{ t('processing.status') }}</th><th>{{ t('processing.stage') }}</th><th>{{ t('processing.detail') }}</th></tr></thead><tbody>
          <tr v-for="row in page.rows" :key="row.row_id"><td><button v-if="row.job_id" class="document-link" @click="selected = row.job_id">{{ row.title || row.external_id || row.job_id }}</button><span v-else>{{ row.title || row.external_id || row.row_id }}</span><small>{{ row.folder_path }}</small><small>{{ row.evidence_basis?.startsWith('legacy') ? label(row.evidence_basis) : row.source_name }}</small></td><td>{{ row.generation ?? '—' }}<small>{{ label(row.availability) }}</small></td><td>{{ label(row.status) }}<small>{{ row.error_code }}</small><small>{{ label(row.event_type) }} {{ label(row.resolution_type) }}</small></td><td>{{ row.stage }}<small v-if="row.step_attempt != null">{{ t('processing.attempt') }}: {{ row.step_attempt }}</small><small v-if="row.next_run_at">{{ t('processing.nextRetry') }}: {{ formatTime(String(row.next_run_at)) }}</small></td><td><small v-for="[key, value] in metrics(row)" :key="String(key)">{{ t(`processing.${key}`) }}: {{ label(value) }}</small><button v-if="typeof row.linked_job_id === 'string' && row.linked_job_id" class="document-link" @click="selected = row.linked_job_id">{{ t('processing.linkedRecovery') }}</button></td></tr>
        </tbody></table></div>
