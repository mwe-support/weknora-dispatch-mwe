...
func TestProcessingKnowledgeCancelFencesPublishedAndCandidateWork(t *testing.T) {
	...
	for _, published := range []bool{false, true} {
		...
		require.ErrorContains(t, err, "processing history controls")
		unchanged, err := r.GetJob(ctx, 1, lease.Job.ID)
		require.NoError(t, err)
		require.NotEqual(t, types.ProcessingCanceled, unchanged.Status)
		request := types.ProcessingControlRequest{ExpectedRevision: unchanged.Revision, OperationRequestID: "cancel-" + file, Actor: "operator", Reason: "synthetic authorized lifecycle control"}
		require.NoError(t, r.CancelJob(ctx, 1, unchanged.ID, request))
		k, err := s.repo.GetKnowledgeByID(ctx, 1, lease.Job.KnowledgeID)
		require.NoError(t, err)
		...
		_, err = s.CancelKnowledgeParse(ctx, lease.Job.KnowledgeID)
		require.ErrorContains(t, err, "processing history controls")
		require.NoError(t, r.CancelJob(ctx, 1, job.ID, request))
