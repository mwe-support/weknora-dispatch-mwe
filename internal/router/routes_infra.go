...
func RegisterProcessingRoutes(r *gin.RouterGroup, h *handler.ProcessingHandler, g *rbacGuards) {
	...
	jobs.POST("/:job_id/retry", g.Admin(), g.KBAccessOwn("id"), h.Retry)
	jobs.POST("/:job_id/pin", g.Admin(), g.KBAccessOwn("id"), h.Pin)
	jobs.POST("/:job_id/rollback", g.Admin(), g.KBAccessOwn("id"), h.Rollback)
	jobs.POST("/:job_id/retire", g.Admin(), g.KBAccessOwn("id"), h.Retire)
	jobs.POST("/:job_id/rebuild", g.Admin(), g.KBAccessOwn("id"), h.Rebuild)
	jobs.POST("/:job_id/cancel", g.Admin(), g.KBAccessOwn("id"), h.Cancel)
	jobs.POST("/:job_id/resolve-export", g.Admin(), g.KBAccessOwn("id"), h.ResolveExport)
